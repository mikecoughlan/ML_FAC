...
DATAPATH = "data/prime/combined/"  # Base data directory
cdas = CdasWs()  # Open connection to CDAS


def load_range(
    dataset_name, var_list, start_date, end_date, load_freq="1MS", time_name="Epoch", resample_freq=None, interp_freq=None, verbose=False, retries=5
):
    ...
    for i in np.arange(len(dates) - 1):
        ...
        while not loaded:
            ...
            try:
                ...
                break
        if loaded:  # Did we successfully load the data in the above while loop?
...
def cdf_to_df_remote(dataset_name, var_list, start_date, end_date, time_name="Epoch"):
    ...
    for var in var_list:
        ...
        if data[1][var].ndim == 2:  # We gotta handle vector data differently because the ending structure must be 2D
            ...
            raise ValueError(
                f"Variable {var} in {dataset_name} has {data[1][var].ndim} dimensions (perhaps it is a distribution function or a constant)."
            )
...
logger.info("Loading FPI ion data.")
...
logger.info("Loading FPI electron data.")
fpi_e_data = load_range(fpi_e_dataset, fpi_e_var_list, datestrs[0], datestrs[1], load_freq="1MS", resample_freq=cadence)
logger.info("Loading MEC data.")
mec_data = load_range(mec_dataset, mec_var_list, datestrs[0], datestrs[1], load_freq="1MS", resample_freq=cadence)
logger.info("Loading FGM data.")
fgm_data = load_range(
    fgm_dataset, fgm_var_list, datestrs[0], datestrs[1], load_freq="1D", resample_freq=cadence
)  # load_freq is shorter here, if longer than ~10D RAM is overloaded. 1D is no slower than 10D
logger.info("Loading SWE data.")
swe_data = load_range(swe_dataset, swe_var_list, datestrs[0], datestrs[1], load_freq="1MS", interp_freq=cadence)
logger.info("Loading FPI data.")
mfi_data = load_range(
    mfi_dataset, mfi_var_list, datestrs[0], datestrs[1], load_freq="1MS", interp_freq=cadence
)  # MFI data is already minutely, it's just on the half-minute so we still have to interp
...
sw_regions["stop"] = pd.to_datetime(
    sw_regions["stop"], utc=True
)  # Stops are on the 59th second of the minute so should be inclusive when binning MMS data
...
sh_regions["stop"] = pd.to_datetime(
    sh_regions["stop"], utc=True
)  # Stops are on the 59th second of the minute so should be inclusive when binning MMS data
...
ms_regions["stop"] = pd.to_datetime(
    ms_regions["stop"], utc=True
)  # Stops are on the 59th second of the minute so should be inclusive when binning MMS data
...
fs_regions["stop"] = pd.to_datetime(
    fs_regions["stop"], utc=True
)  # Stops are on the 59th second of the minute so should be inclusive when binning MMS data

# Create a dataframe to put all the data into
combo_df = pd.DataFrame([])
combo_df["Epoch"] = pd.date_range(
    mms_labels["Epoch"].min(), mms_labels["Epoch"].max(), freq="1min"
)  # Make a time for every minute so gaps can be identified as nans
...
logger.info("Marking stable regions.")
...
for region_df in [sw_regions, sh_regions, ms_regions, fs_regions]:
    for idx in region_df.index:  # Loop over rows in the region dataframe to assess stability
        ...
        combo_df.loc[(combo_df["Epoch"] >= region["start"]) & (combo_df["Epoch"] <= region["stop"]), "stable"] = 1

# Save the combined dataframe to an HDF
logger.info("Saving combined dataframe.")
combo_df.to_hdf(DATAPATH + "combined_data.h5", key="mms_wind_1min")
