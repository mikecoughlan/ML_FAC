...
def fit_model(model, empty_model, train, val, val_loss_patience=25, num_epochs=500, bin_edges=None, bin_weights=None, model_config=None):
    ...
    if not finished_training:
        ...
        while current_epoch < num_epochs:
            ...
            print(
                f"Epoch [{current_epoch}/{num_epochs}], Loss: {loss:.4f} Validation Loss: {val_loss:.4f}" + f" Epoch Time: {epoch_time:.2f} seconds"
            )
...
def main():
    ...
    with open(
        working_dir + f"/outputs/{CONFIG['model']}_{CONFIG['version']}_{CONFIG['eras']}_storm_training_{CONFIG['extract_storms']}_results.pkl", "wb"
    ) as f:
