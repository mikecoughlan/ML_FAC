...
class PreparingData:
    ...
    def __init__(
        ...
        **kwargs,
        ...
        # ---------------------------------------------------------------------
        try:
            with open(f"{config}_config.json", "r") as f:
                self.config = json.load(f)
        except FileNotFoundError:
            raise FileNotFoundError(f"Configuration file '{config}_config.json' not found. Please ensure the file exists in the working directory.")
    ...
    def loading_solarwind(self):
        ...
        # ---------------------------------------------------------
        sw_path = self.data_dir + self.config.get("sw_data", "sw_data/omni/omni_10_min_interp.feather")
        ...
        # -------------------------------------------------------------
        f107_path = self.data_dir + self.config.get("F107_data", "sw_data/F107/fluxtable.txt")
        indicies_path = self.data_dir + self.config.get("indicies_data", "indicies/supermag_indicies.feather")
        ...
        daily_f107 = self.F107.groupby("fluxdate")["F107"].mean()

        # Convert 'fluxdate' into a DatetimeIndex (+20:00 shift)
        daily_f107.index = pd.to_datetime(daily_f107.index, format="%Y%m%d") + datetime.timedelta(hours=20)
        ...
        if self.eras == "block_1":
            self.solarwind = self.solarwind[self.solarwind.index < "2018-01-01 00:00:00"]
    ...
    def day_of_year_to_month_day(self, year, day_of_year, fractional_hour):
        ...
        date = (
            ...
            + datetime.timedelta(days=int(day_of_year) - 1)
            + datetime.timedelta(hours=hours, minutes=minutes, seconds=int(0))
        )
        return (
            str(date.year)
            + "-"
    ...
    def unpacking_current_density(self, file: str | Path, pivot_or_array: str = "pivot") -> dict:
        """
        ...
        try:
            ...
            for hour in range(n_hours):
                ...
                timestamp = self.day_of_year_to_month_day(years[hour], doys[hour], times[hour])
                ...
                if pivot_or_array == "array":
                    ...
                    df = pd.DataFrame({"current_density": jpar[hour, :], "mlt": mlt[hour, :], "lat": lat[hour, :]})
                    # df['current_density'][df['current_density'] > 1e30] = np.nan
                    current_density_dict[timestamp] = df.pivot_table(index="lat", columns="mlt", values="current_density")
    ...
    def checking_for_storm(self, i: int, param_values: np.ndarray) -> Tuple[int, Optional[pd.Series]]:
        ...
        while i < n:
            ...
            condition_met = (current_value <= self.storm_extract_limit and self.storm_extract_limit < 0) or (
    ...
    def storm_extract(self) -> Tuple[pd.DataFrame, List[pd.DataFrame]]:
        ...
        if not self.use_disturbed_time_list:
            ...
            while i < n:
                ...
                potential_start = (current_value <= self.storm_extract_limit and self.storm_extract_limit < 0) or (
                    current_value >= self.storm_extract_limit and self.storm_extract_limit > 0
                )
    ...
    def processing(self):
        ...
        if self.extract_storms:
            ...
            self.ampere = {key: val for key, val in tqdm.tqdm(self.ampere.items()) if key in segmented_timestamps_index}
        else:
            ...
            if self.eras == "block_1":
                ...
                start_date = "2009-07-01"
                end_date = "2025-08-01"
            segmented_list = pd.date_range(start=pd.to_datetime(start_date), end=pd.to_datetime(end_date), freq="MS").tolist()
        ...
        if os.path.exists(split_path):
            ...
            merged_dict = {
                key: {"input": omni_dict[key], "ampere": self.ampere[key]}
                for key in common_keys
                if (
                    not np.isnan(np.asarray(omni_dict[key], dtype=float)).any()
                    and not np.isnan(np.asarray(self.ampere[key], dtype=float)).any()
        ...
        if self.specific_test_storms:
            ...
            print("Checking if this specific storms were extracted for testing:")
        ...
        if not self.extract_storms:
            ...
            train_date_range = pd.concat(
                [pd.Series(pd.date_range(start=t, end=t + pd.offsets.MonthBegin(), freq="min", inclusive="left")) for t in train_times]
            )

            val_date_range = pd.concat(
                [pd.Series(pd.date_range(start=t, end=t + pd.offsets.MonthBegin(), freq="min", inclusive="left")) for t in val_times]
            )

            test_date_range = pd.concat(
                [pd.Series(pd.date_range(start=t, end=t + pd.offsets.MonthBegin(), freq="min", inclusive="left")) for t in test_times]
            )
    ...
    def __call__(self):
        ...
        if os.path.exists(self.data_dir + f"prepared_data/fully_prepared_{self.eras}_{self.data_version}_disturbed_time_{self.extract_storms}.pkl"):
            ...
            with open(
                self.data_dir + f"prepared_data/fully_prepared_{self.eras}_{self.data_version}_disturbed_time_{self.extract_storms}.pkl", "rb"
            ) as f:
                data = pickle.load(f)
            train = data["train"]
            val = data["val"]
            ...
            with open(
                self.data_dir + f"prepared_data/fully_prepared_{self.eras}_{self.data_version}_disturbed_time_{self.extract_storms}.pkl", "wb"
            ) as f:
