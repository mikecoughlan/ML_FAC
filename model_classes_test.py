...
def _conv_pad(in_channels: int, out_channels: int, kernel_size: int) -> nn.Module:
    """
    Conv2d that always preserves spatial dimensions regardless of kernel parity.

    Odd kernels (1, 3, 5, …): symmetric padding = kernel_size // 2, handled
    directly by Conv2d.

    Even kernels (2, 4, 6, …): Conv2d with padding=0, preceded by ZeroPad2d
    that pads (left, right, top, bottom) = (p, p-1, p, p-1) where p = k // 2.
    This crops the extra pixel that symmetric padding would otherwise add,
    keeping output H and W identical to input H and W.
    """
    if kernel_size % 2 == 1:
...
class ResidualBlock(nn.Module):
    ...
    def __init__(
        ...
        for i in range(num_layers):
            ...
            layers += [
                _conv_pad(ch_in, out_channels, kernel_size),
